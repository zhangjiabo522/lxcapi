...
# ============================================================================
# LxcAPI 节点安装管理脚本
# 将 Ubuntu / Debian 服务器配置为 LxcAPI 面板管理的 Incus LXC 容器节点
#
# 用法:
#   交互式: sudo bash LxcAPI.sh
...
readonly BRIDGE_NAME="lxcbr0"
readonly PRESEED_FILE="/tmp/.lxcapi-preseed-$$.yaml"
...
readonly AGENT_SERVICE_NAME="lxcapi-agent"
readonly AGENT_CONFIG_FILE="${INCUDAL_AGENT_CONFIG_FILE:-/etc/lxcapi-agent/config.yaml}"
readonly AGENT_BIN_PATH="${INCUDAL_AGENT_BIN:-/usr/local/bin/lxcapi-agent}"
...
show_banner() {
    ...
    echo -e "${CYAN}  ╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}  ║                                                  ║${NC}"
    echo -e "${CYAN}  ║            ${BOLD}LxcAPI 节点安装管理脚本${NC}${CYAN}              ║${NC}"
...
setup_warp_v4() {
    ...
    echo "$REAL_V6_IFACE" > /etc/lxcapi_v6_iface
    echo "$REAL_V6_GW" > /etc/lxcapi_v6_gw
    ...
    # 注入出站控制器
    cat > /usr/local/bin/lxcapi-warp << 'EOF'
#!/bin/bash
if [[ "$1" == "on" ]]; then
    ...
    REAL_V6_IFACE=$(cat /etc/lxcapi_v6_iface 2>/dev/null)
    REAL_V6_GW=$(cat /etc/lxcapi_v6_gw 2>/dev/null)
    [ -z "$REAL_V6_IFACE" ] && REAL_V6_IFACE=$(ip -o -6 addr show scope global | grep -v wg | awk '{print $2}' | head -1)
...
# LxcAPI WARP-OFF 模式 - 纯 NAT64+DNS64 解析器
...
DNSEOF
    ...
    echo -e "用法: \033[1;36mlxcapi-warp [on/off]\033[0m"
...
EOF
    chmod +x /usr/local/bin/lxcapi-warp
    info "出站调度指令控制台部署成功: 随时键入 lxcapi-warp 操控"

    # 部署 IPv6 路由恢复守护脚本（兜底保险）
    cat > /usr/local/bin/ipv6-route-guard.sh << 'GUARD_EOF'
#!/bin/bash
# IPv6 路由恢复守护脚本 - 由 LxcAPI 安装脚本自动部署
# 每 10 秒检测 IPv6 默认路由，丢失则自动恢复

IPV6_IFACE=$(cat /etc/lxcapi_v6_iface 2>/dev/null)
IPV6_GATEWAY=$(cat /etc/lxcapi_v6_gw 2>/dev/null)
...
[Unit]
Description=IPv6 Route Guard - LxcAPI 自动恢复 IPv6 默认路由
...
setup_kernel() {
    ...
    cat > /etc/sysctl.d/99-lxcapi.conf <<EOF
# LxcAPI 节点内核参数 - 由安装脚本自动生成
# ======== 文件系统 ========
...
update_lxcapi_agent_from_config() {
    local panel_url=""
    local agent_id=""
    local agent_secret=""
    local heartbeat_interval=""
    ...
    run_lxcapi_agent_installer "$panel_url" "" "$agent_id" "$agent_secret" "$AGENT_HEARTBEAT_INTERVAL_SECONDS" "$AGENT_BINARY_URL"
}

install_lxcapi_agent_with_token() {
    echo ""
    divider
    ...
    run_lxcapi_agent_installer "$panel_url" "$install_token" "" "" "$AGENT_HEARTBEAT_INTERVAL_SECONDS" "$AGENT_BINARY_URL"
}

show_lxcapi_agent_logs() {
    step "查看 Agent 最近日志"
...
manage_lxcapi_agent() {
    while true; do
        ...
        case "$choice" in
            1) update_lxcapi_agent_from_config || true; pause_return ;;
            2) install_lxcapi_agent_with_token || true; pause_return ;;
            3) show_lxcapi_agent_logs || true; pause_return ;;
            0) return 0 ;;
            *) warn "无效选项，请重新选择" ;;
        esac
    done
...
show_result() {
    ...
    if [[ -f "$script_path" && ! "$script_path" =~ (bash|sh)$ ]]; then
        # 正常文件执行：直接复制脚本
        cp -f "$script_path" /usr/local/bin/lxcapi 2>/dev/null || true
        chmod +x /usr/local/bin/lxcapi 2>/dev/null || true
    elif [[ -f /root/lxcapi.sh ]]; then
        # 回退方案 1：检查常见的下载位置
        cp -f /root/lxcapi.sh /usr/local/bin/lxcapi 2>/dev/null || true
        chmod +x /usr/local/bin/lxcapi 2>/dev/null || true
    else
        # 回退方案 2：创建自下载包装器，运行时从面板拉取最新脚本
        cat > /usr/local/bin/lxcapi <<'SHORTCUT'
#!/bin/bash
# LxcAPI 节点管理快捷入口 - 自动下载最新版本
SCRIPT_CACHE="/root/lxcapi.sh"
SCRIPT_URL="https://raw.githubusercontent.com/0xdabiaoge/lxcapi/main/server/templates/install.sh"
echo "正在获取最新的节点管理脚本..."
if curl -sSfL "$SCRIPT_URL" -o "$SCRIPT_CACHE" 2>/dev/null; then
    ...
    echo "或手动从面板获取安装脚本保存到 /root/lxcapi.sh"
    exit 1
fi
SHORTCUT
        chmod +x /usr/local/bin/lxcapi 2>/dev/null || true
    ...
    echo -e "  快捷命令  :  ${GREEN}lxcapi${NC}"
    echo ""
    divider
    echo -e "  ${BOLD}下一步：${NC}请返回 LxcAPI 面板，点击「验证并连接」按钮完成注册"
    echo -e "  ${DIM}随时输入 lxcapi 可重新进入此管理脚本${NC}"
    divider
    echo ""
}
...
do_uninstall() {
    ...
    if ip link show wg0 >/dev/null 2>&1 || [[ -f /usr/local/bin/wgcf ]] || [[ -d /etc/wireguard ]]; then
        ...
        rm -f /usr/local/bin/wgcf 2>/dev/null || true
        rm -f /usr/local/bin/lxcapi-warp 2>/dev/null || true
    ...
    # 3. 清除 IPv6 同步守护神服务
    if systemctl list-unit-files 2>/dev/null | grep -q "lxcapi-v6-guardian.service"; then
        systemctl stop lxcapi-v6-guardian 2>/dev/null || true
        systemctl disable lxcapi-v6-guardian 2>/dev/null || true
        rm -f /etc/systemd/system/lxcapi-v6-guardian.service
        rm -f /usr/local/bin/lxcapi-v6-guardian.sh
        systemctl daemon-reload
    ...
    # 内核参数配置
    if [[ -f /etc/sysctl.d/99-lxcapi.conf ]]; then
        rm -f /etc/sysctl.d/99-lxcapi.conf
        info "已删除: /etc/sysctl.d/99-lxcapi.conf"
    fi
    ...
    rm -f lxcapi-install.sh *.install.sh install.sh lxcapi.sh 2>/dev/null || true
    rm -f /root/install.sh /root/lxcapi.sh 2>/dev/null || true
}
...
setup_v6_guardian() {
    ...
    local guardian_script="/usr/local/bin/lxcapi-v6-guardian.sh"
    local guardian_service="/etc/systemd/system/lxcapi-v6-guardian.service"
    
    cat > "$guardian_script" << 'EOF'
#!/bin/bash
# LxcAPI - IPv6 Dual-Stack Guardian Daemon
...
while true; do
  ...
  if [[ -f /etc/sysctl.d/99-lxcapi.conf ]]; then
      sysctl -p /etc/sysctl.d/99-lxcapi.conf >/dev/null 2>&1 || true
  fi
...
[Unit]
Description=LxcAPI IPv6 Dual-Stack Guardian Daemon
After=network.target
...
EOF
    ...
    systemctl daemon-reload 2>/dev/null || true
    systemctl enable lxcapi-v6-guardian 2>/dev/null || true
    systemctl restart lxcapi-v6-guardian 2>/dev/null || true
    ...
    if [[ -z "$MODE" ]]; then
        ...
        while true; do
            ...
            case "$choice" in
                ...
                8)  manage_lxcapi_agent; continue ;;
                0)  info "再见！"; exit 0 ;;
